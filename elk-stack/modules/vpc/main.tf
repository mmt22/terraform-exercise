...
resource "aws_route_table" "k8infra-public-RT" {
  vpc_id = aws_vpc.main.id
...
resource "aws_route_table_association" "k8-public-subnet-1" {
  subnet_id      = aws_subnet.pubsubnet-1.id
  route_table_id = aws_route_table.k8infra-public-RT.id
}

resource "aws_route_table_association" "k8-public-subnet-2" {
  subnet_id      = aws_subnet.pubsubnet-2.id
  route_table_id = aws_route_table.k8infra-public-RT.id
}

resource "aws_route_table_association" "k8-private-subnet-1" {
  subnet_id      = aws_subnet.privatesubnet-1.id
  route_table_id = aws_route_table.private.id
}

resource "aws_route_table_association" "k8-private-subnet-2" {
